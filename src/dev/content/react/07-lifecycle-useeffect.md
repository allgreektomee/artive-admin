...
1. **의존성 `[loadVersion]`**  
   ...
   - **화면으로는** 잠깐 `로딩 중…`이 보였다가 다시 두 줄 목록이 나온다. 끝 상태가 항상 “두 줄”이라 **`setItems([...])`만 보면 결과가 하나처럼** 느껴질 수 있다. Live에서는 목록 끝의 **(불러오기 #N)** 과 **① effect 실행 횟수**로 재실행을 확인한다.  
   - 이 과정은 **전체 컴포넌트 리마운트가 아니라**, `loadVersion`에 매달린 effect만 한 바퀴 더 도는 것이다(클래스의 **componentDidUpdate** 류에 가깝다).
