...
다른 파일로 나누지 않고 **이 Live와 같은 단일 컴포넌트** 안에서만 아래를 스스로 해 보면, 5장 개념이 몸에 붙는다.

1. **숫자 state** 하나를 `+` / `−` 버튼으로 바꾸고, 화면에 같은 값이 보이는지 확인한다.  
2. **불리언 state** 하나로 문구나 스타일(예: “공개/비공개”)을 토글한다.  
3. **문자열 state**와 `<input value={...} onChange={...}>`로 **제어 컴포넌트**를 만든다.  

한 컴포넌트가 길어지면 6장에서처럼 **조각으로 나누고 props로 연결**하면 된다.
