...
- 총 15장: **1~8장 = Live 예제 8개와 동일 순서**, 9~14장 = 라우터·API·인증(개념)·폼·Redux·이미지, **15장 WebSocket = 선택**. 통합 흐름은 `reactTestProject` 데모로 이어진다.
...
- `reactTestProject` 참고 트리와 [`/dev/react-test/artworks`](/dev/react-test/artworks) 데모로 목록·상세·업로드 흐름을 묶어 본다. (WebSocket은 생략 가능.)
