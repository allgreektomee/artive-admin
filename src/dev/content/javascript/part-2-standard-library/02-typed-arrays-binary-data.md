...
- 엔디언은 여러 바이트로 된 숫자를 앞쪽 바이트부터 읽을지, 뒤쪽 바이트부터 읽을지 정하는 바이트 순서다.
- 일반 서비스 코드에서는 자주 쓰지 않지만, 파일·이미지·암호화·네트워크·Web API를 다룰 때 중요하다.

## 용어 먼저 정리

이진 데이터는 0과 1로 표현되는 원시 데이터다. 파일, 이미지, 오디오, 네트워크 패킷은 결국 바이트 단위의 이진 데이터로 다뤄진다.

바이트는 8비트 묶음이다. `Uint8Array`의 `8`은 8비트, 즉 1바이트 단위로 값을 읽고 쓴다는 뜻이다.

엔디언은 2바이트 이상 숫자를 메모리에 어떤 순서로 놓고 읽을지에 대한 규칙이다. 리틀 엔디언은 낮은 자리 바이트를 먼저 두고, 빅 엔디언은 높은 자리 바이트를 먼저 둔다. 일반 배열을 다룰 때는 거의 신경 쓰지 않지만, `DataView`로 바이너리 포맷을 직접 읽을 때 중요하다.
