...
객체와 배열은 JavaScript에서 데이터를 담는 가장 중요한 구조다. 여기서는 실제 코드에서 객체와 배열을 어떻게 읽고, 바꾸고, 순회하는지 정리한다.
...
- 객체 리터럴은 `{}`로 객체 값을 직접 쓰는 표현이다.
- 프로퍼티는 객체 안에 들어 있는 키와 값의 한 쌍이다.
- 점 표기법은 정적인 프로퍼티에 적합하다.
- 대괄호 표기법은 프로퍼티 이름이 변수일 때 필요하다.
- 구조 분해 할당은 배열이나 객체에서 필요한 값을 꺼내 변수로 받는 문법이다.
- `Object.entries()`는 키와 값을 함께 순회할 때 편하다.

## 용어 먼저 정리

객체 리터럴은 코드에 객체 값을 직접 써놓은 표현이다.
...
`name: 'Kim'`처럼 객체 안에 들어 있는 키와 값의 한 쌍을 프로퍼티라고 한다. 점 표기법은 `user.name`처럼 프로퍼티 이름을 코드에 직접 쓸 때 사용한다. 대괄호 표기법은 프로퍼티 이름이 변수에 들어 있을 때 사용한다.
...
구조 분해 할당은 배열이나 객체에서 값을 꺼내 변수에 바로 담는 문법이다.

```js
const entry = ['name', 'Kim'];
...
아래 `Object.entries()` 예제의 `[key, value]`도 구조 분해 할당이다.
