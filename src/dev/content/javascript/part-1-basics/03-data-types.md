...
`symbol`은 충돌하지 않는 고유한 키를 만들 때 쓰는 타입이다. 객체 내부의 특별한 키나 표준 동작을 정의할 때 자주 등장한다. 자세한 활용은 2부의 심벌 글에서 다룬다.
...
`bigint`는 `number`로 안전하게 다루기 어려운 큰 정수를 표현하는 타입이다. 숫자 뒤에 `n`을 붙여 만든다.
