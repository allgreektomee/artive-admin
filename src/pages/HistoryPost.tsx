...
import { Form, Input, Button, Switch, Select, Card, message, Divider, Upload, Spin, type DividerProps } from "antd";
...
const HistoryPost: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    if (!finalImageUrl) {
      return message.warning(
        values.type === "IMGURL" ? "이미지를 업로드해주세요." : "링크 주소를 입력해주세요."
      );
    }
  ...
  return (
    <Card title="📜 작업 기록 추가" style={{ maxWidth: 800, margin: "20px auto" }}>
      <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ isPublic: true, type: "IMGURL" }}>
        ...
        <div style={{ display: "flex", gap: "16px" }}>
          <Form.Item name="type" label="기록 유형" style={{ flex: 1 }}>
            <Select 
              onChange={() => { setS3Url(""); form.setFieldValue("imageUrl", ""); }}
              options={[
        ...
        {/* 🖼️ 타입별 입력 UI 분기 */}
        <div style={{ padding: '20px', background: '#f9f9f9', borderRadius: '8px', border: '1px solid #eee', marginBottom: 24 }}>
          {historyType === "IMGURL" ? (
                ...
                >
                  {s3Url ? (
                    <div style={{ padding: '10px' }}>
                      <img src={s3Url} alt="업로드 미리보기" style={{ maxWidth: '100%', maxHeight: '200px', borderRadius: '4px' }} />
                      <p style={{ marginTop: 8, color: '#1890ff' }}>파일 교체하려면 클릭하거나 드래그하세요</p>
                    </div>
                  ) : (
                    <>
                      <p className="ant-upload-drag-icon"><PictureOutlined /></p>
                      <p className="ant-upload-text">이미지 파일을 이곳에 드래그하거나 클릭하세요</p>
                      <p className="ant-upload-hint">1개 파일만 업로드 가능합니다 (S3 저장)</p>
                    </>
                  )}
                </Dragger>
              </Spin>
            ...
            <Form.Item 
              name="imageUrl" 
              label={historyType === "YOUTUBE" ? "유튜브 영상 URL" : "외부 참조 URL"} 
        ...
        <Button type="primary" htmlType="submit" loading={saveLoading} block size="large">
          기록 저장하기
        </Button>
      </Form>
    </Card>
